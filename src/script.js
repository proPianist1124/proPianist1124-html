main()
async function main(){
    const projects_response = await fetch("src/data/projects.json")
    ...
    for(let i=0; i<socials_data.socials.length; i++){
        ...
        socials.innerHTML = `${socials.innerHTML}<a href = "${socials_data.socials[i].url}"><button class = "${socials_data.socials[i].class}"><i class = "${socials_data.socials[i].icon}"></i> ${socials_data.socials[i].name}</button></a><br>`
    }
}